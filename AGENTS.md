...
reasoning lives in the maintainer's working notes (`local/docs/PROJECT-PLAN.md` and
`local/docs/specs/`), which are **not committed**. Everything a contributor needs to
follow the rules is in this file.
...
8. SRD compendium + custom-creature/spell form. The compendium ships as static
   JSON in `console/public/compendium/`; **the ingest tooling lives in the separate
   ...
   **Before touching the compendium data or the stat-block UI, read
   ...
   each one**: give it an entry in `console/src/combat/spells/*` or list it, with a
   reason, in `console/tests/combat/spellCoverage.data.ts`. `spellCoverage.test.ts`
   fails until every
...
- **Where they live:** the console's suite is `console/tests/`, mirroring
  `console/src/` (never co-located). The site's is `site/tests/`, mirroring
  `site/src/`. The deploy scripts are covered from `tests/scripts/` in this repo.
- **How they run:** `npm run test` at this root chains all three suites. Tests
  default to the fast node
...
- **Spells are gated:** every spell in a shipped library needs a verdict, either an
  implementation in `console/src/combat/spells/*` or a reasoned entry in
  `console/tests/combat/spellCoverage.data.ts` — or `spellCoverage.test.ts` fails.
...
- **A change in a part is committed in that part's repo.** Commits here are submodule
  pointer bumps, deploy scripts, screenshots, and the shared docs.
